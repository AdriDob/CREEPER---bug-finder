...
from core.targets import parser, scorer, filters
...
class Hunter:
    ...
    def fetch_public_programs(self, platform: str, limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            resp = requests.get(url, timeout=10, headers={"User-Agent": "rastro/1.0 (+https://rastro.local)"})
    ...
    def ingest_programs(self, programs: List[Dict]) -> List[Dict]:
        ...
        try:
            for p in programs:
                ...
                multi_tenant = any(k in keywords for k in ["org_id", "tenant", "workspace", "account_id", "team_id"]) or wildcard
                ...
                scores = scorer.score_target(meta)
                ...
                intel = TargetIntel(
                    ...
                    noise_score=scores["noise_level"],
